...
func formatHapticNumber(_ num: Int) -> String {
    // Use 999_950 threshold to avoid "1000.0K" from %.1f rounding 999,999
    if num >= 999_950 {
...
func formatItemNumber(_ num: Int) -> String {
    let abs = abs(num)
    let sign = num < 0 ? "-" : ""
    // Use 999_950 threshold to avoid "1000.0K" from %.1f rounding
    if abs >= 999_950 {
        return sign + String(format: "%.1fM", Double(abs) / 1_000_000)
    } else if abs >= 10_000 {
        return sign + String(format: "%.1fK", Double(abs) / 1_000)
    }
    return "\(num)"
}
